...
public class SpotifyExplorer {
    ...
    public SpotifyExplorer(Reader dataInput) {
        try (BufferedReader reader = new BufferedReader(dataInput)) {
            reader.readLine();   //removes the first line of the given file
            tracks = reader.lines()
    ...
    public Collection<SpotifyTrack> getExplicitSpotifyTracks() {
        ...
        Collection<SpotifyTrack> explicitOnly = tracks.stream()
                .filter(SpotifyTrack::explicit)
                .collect(Collectors.toList());
        return Collections.unmodifiableCollection(explicitOnly);
    }
    ...
    public int getArtistActiveYears(String artist) {
        ...
        int oldest = tracks.stream()
                .filter(name -> name.artists().contains(artist))
                .mapToInt(SpotifyTrack::year)
                .min()
                .orElse(0);

        int latest = tracks.stream()
                .filter(name -> name.artists().contains(artist))
                .mapToInt(SpotifyTrack::year)
                .max()
                .orElse(0);
    ...
    public List<SpotifyTrack> getTopNHighestValenceTracksFromThe80s(int n) {
        if (n < 0) {
            throw new IllegalArgumentException("Cannot get " + n + " tracks with hightest valence");
        }

        List<SpotifyTrack> highEnergySongs = tracks.stream()
                .filter(song -> song.year() >= 1980 && song.year() <= 1989)
                .sorted(Comparator.comparing(SpotifyTrack::valence).reversed())
                .limit(n)
                .collect(Collectors.toList());

        return Collections.unmodifiableList(highEnergySongs);
    }
    ...
    public long getNumberOfLongerTracksBeforeYear(int minutes, int year) {
        if (minutes < 0) {
            throw new IllegalArgumentException("Negative minutes are not allowed");
        }
        if (year < 0) {
            throw new IllegalArgumentException("No current record of tracks from the year" + year);
        }

        long minutesToMs = minutes * 60_000;
        return tracks.stream()
                .filter(song -> song.duration() > minutesToMs && song.year() < year)
                .count();
    }
    ...
    public Optional<SpotifyTrack> getTheLoudestTrackInYear(int year) {
        if (year < 0) {
            throw new IllegalArgumentException("No current record of tracks BC");
        }
        return tracks.stream()
